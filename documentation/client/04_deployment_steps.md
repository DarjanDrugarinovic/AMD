...
This installs `vite-tsconfig-paths` and `gh-pages` from `devDependencies` and locks them into `package-lock.json` so the CI install step can find them.
...
No custom secrets are needed — `GITHUB_TOKEN` and `GITHUB_REPOSITORY` are provided automatically by GitHub Actions.
